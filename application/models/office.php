...
foreach ($codes->result() as $row) {
        ...
        $arr['flag']="<div class='hint'>". t('yourPostal') ."</div>";
        $arr['more']= t('more-info');
          $arr['id']  =$temp->id;
          $arr['postal_code']  =$temp->postal_code;
          $temp->AddPostOfficePrefix();
          $arr['office'] = $temp->office;
          $temp->AddBaseOfficeURL();  
          $arr['route'] =$temp->route;
	     array_push($response['results'] , $arr);
}
...
function search($query){
  ...
  $response['results'] = array();
    ...
    $this->db->select('office, address, route');
    $this->db->like('office', $query);
    $this->db->like('address', $query);
    $offices= $this->db->get($this::DB_TABLE,10,0); //min:0 max:10
...
function AddPostOfficePrefix(){
  $this->office = t('post-office') ." ". $this->office ;
}
function AddPostNumberPrefix(){
  $this->office = t('zipCode') . $this->office ;
}
...
$this->load->helper('language');
$this->lang->load('home_page', $this->config->item('language'));
$sec_display= array( t('great_cairo'), t('north_delta') , t('east_delta'), t('west_delta') );
 $result = array();

$result['sections']=array();

$result['title']=array();
$result['title']=$sec_display;
